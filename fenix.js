...
async function extrairTexto() {
  ...
  try {
    ...
    const { data: { text } } = await Tesseract.recognize(preview.src, 'por', {
    ...
    showError("Erro ao extrair texto da imagem.");
  }
}
...
function detectarCores() {
  ...
  for (let i = 0; i < imageData.length; i += 40) {
    ...
    if (cores.size >= 3) break;
  }
  const coresLista = cores.size > 0 ? Array.from(cores).join(", ") : "Nenhuma cor dominante.";
...
function reconhecerObjetos() {
  ...
  // Simulação: lista fixa de objetos (sem API de visão)
  const objetosPossiveis = ['Livro', 'Caneta', 'Mesa', 'Cadeira', 'Árvore', 'Carro'];
  const objetos = objetosPossiveis.slice(0, Math.floor(Math.random() * 3) + 1);
...
function toggleMic() {
  ...
  // Ativar modo conversa automaticamente
  if (!chatMode) {
    toggleChatMode();
  }
  const btnFalar = document.getElementById('btnFalar');
...
/* System: It looks like the `fenix.js` artifact is incomplete, as the `processarAventura` function is cut off mid-logic. I'll complete the artifact by finishing the `processarAventura` function and ensuring all other functionalities (text extraction, color detection, object recognition, and voice-to-chat) are intact. I'll also maintain the same `artifact_id` and update the `artifact_version_id` to reflect this correction. The `index.html`, `fenix.css`, and JSON files are unchanged since you confirmed the visual is okay and the JSONs are functional.

### Completing the `fenix.js` Artifact
I'll finalize the `processarAventura` function to handle the "norte" and "sul" choices correctly, ensuring the adventure state is reset and the UI updates. The rest of the code will remain as provided, with Tesseract.js for text extraction, canvas-based color detection, simulated object recognition, and the modified `toggleMic` to activate chat mode.

#### `fenix.js` (Completed)
<xaiArtifact artifact_id="eba49c66-3ac7-411e-90ab-7808959ab6a7" artifact_version_id="8c1809e4-845d-4ef9-ab55-c1c900819a08" title="fenix.js" contentType="text/javascript">
console.log("Iniciando fenix.js...");
